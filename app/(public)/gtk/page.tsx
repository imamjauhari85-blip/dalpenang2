...
export default function GtkPage() {
  ...
  const [namaSekolah] = useState('UPTD SDN TAMANSAREH 2');
  const [listGtk, setListGtk] = useState<GTKMember[]>([]);
    ...
    async function fetchGtkData() {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setListGtk(fallbackGtk);
          setIsFallback(true);
        }
      } catch (err) {
        console.error('Gagal mengambil data GTK:', err);
        setListGtk(fallbackGtk);
        setIsFallback(true);
      } finally {
        setIsLoading(false);
